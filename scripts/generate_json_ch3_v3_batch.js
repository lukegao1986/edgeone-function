...
- phy_1_1_6: (6)摩擦や空気の抵抗を受ける運動 (静止摩擦力,動摩擦力,空気の抵抗と終端速度)
  ...
  let currentMaxIndexes = await getMaxIndexes(chapterStr);
  console.log(`📊 初始最大编号: C:${currentMaxIndexes.C}, E:${currentMaxIndexes.E}, B:${currentMaxIndexes.B}, A:${currentMaxIndexes.A}`);

  // 从 part 2 开始处理 (因为 part 1 已经跑过了，为了不重复，这里直接从 1 跑到 4。数组下标 1-4 代表 part 2 到 part 5)
  // 如果需要全量重新跑，可以把循环改为 0 到 partsCount - 1
  for (let partIndex = 1; partIndex < partsCount; partIndex++) {
    console.log(`\n\n===========================================`);
    console.log(`🚀 开始处理 Part ${partIndex + 1} / ${partsCount}`);
    ...
    const startC = currentMaxIndexes.C + 1;
    const startE = currentMaxIndexes.E + 1;
    const startB = currentMaxIndexes.B + 1;
    const startA = currentMaxIndexes.A + 1;

    const prompt = `你是一个 EJU（日本留学試験）物理题库编辑专家。

请阅读以下输入文件，将教材中的练习题转换为标准 JSON 题库格式。由于题目较多，这是第 ${partIndex + 1} 批数据。请确保返回格式是完整的 JSON 数组！
...
2. **匹配 Syllabus 内容**：将提取到的关键字与提供的 Syllabus 列表の \`title\` 或 \`content\` 进行比对。例如“垂直抗力”对应 \`phy_1_1_2\` 中包含の“抗力”。
3. **题干兜底判断**：如果题号后没有明确的关键字，请通过阅读题干の物理情景和求解目标（如求合力、张力等），结合物理本质进行合理归类。
...
3. 干扰选项的生成策略：
   ...
   - 不要用"以上都不对"之类的选项
4. 多问题（N≥2）の选项是各小问答案の组合
   - 每个选项包含所有小问の答案，用逗号或分号分隔
   - 示例（N=2）："5.0 m/s, 9.0 m" 表示 (1)の答案=5.0, (2)の答案=9.0
   - 组合策略：为每问设置 2-3 个候选值（1 正确 + 1~2 干扰），交叉组合出 2N+2 个选项
5. correctIndex 为单个整数，指向正确组合选项の索引（0-based）。请务必**随机打乱**选项顺序，并确保 correctIndex の取值是随机分布在 0 到 2N+1 之间，绝对不要让所有的题目的 correctIndex 都是 0！
...
- 保留 LaTeX 公式格式
- **【强制语言要求】解析解说（包括解题思路、步骤、公式推导说明等）必须全程使用【纯中文】撰写**。题干保持日文原文，但解析内容坚决不允许使用日文（专有名词除外）。
...
请返回严格的 JSON 数组格式。`;
    ...
    console.log(`🤖 Calling DeepSeek API for Part ${partIndex + 1}...`);
    ...
    try {
      ...
      try {
        ...
        parsed = rawParsed.map(q => {
          // 更新最大序号计数器，方便下一个 Part 继续递增
          const match = q.businessCode.match(/q([CEBA])(\d+)$/);
          if (match) {
            const type = match[1];
            const num = parseInt(match[2], 10);
            if (num > currentMaxIndexes[type]) {
              currentMaxIndexes[type] = num;
            }
          }
        ...
        console.error(`❌ Failed to parse JSON for Part ${partIndex + 1}:`, e);
        fs.writeFileSync(outputFile + '.raw', jsonStr);
        continue; // 继续跑下一个 Part
      }
      ...
      console.log(`📈 当前内存中最大编号已更新: C:${currentMaxIndexes.C}, E:${currentMaxIndexes.E}, B:${currentMaxIndexes.B}, A:${currentMaxIndexes.A}`);
      
    } catch (err) {
      console.error(`❌ API Error in Part ${partIndex + 1}:`, err.message);
    }
  }
  console.log('\n🎉 所有剩余批次处理完毕！');
}
